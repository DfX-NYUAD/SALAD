...
class QADataset(Dataset):
    def __init__(
        ...
        # print("🧪 Before fix:", hf_args)
        # hf_args["path"] = "json"
        # hf_args["data_files"] = {"train": "/scratch/zw3464/open-unlearning/RTL_dataset/verilogeval.json"}
        # hf_args["split"] = "train"
        # print("Forced fix applied:", hf_args)
        ...
        if isinstance(data, DatasetDict):
            if len(data) == 1:
                ...
                raise ValueError(f"Expected one split in DatasetDict, got: {list(data.keys())}")
        self.data = add_dataset_index(data)
        self.fs_data = None
