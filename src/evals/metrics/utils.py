...
#     # Shift for causal loss computation
#     shifted_logits = logits[..., :-1, :].contiguous()  # (B, T-1, V)
#     shifted_labels = labels[..., 1:].contiguous()      # (B, T-1)

#     # Flatten for CrossEntropyLoss
#     loss_function = nn.CrossEntropyLoss(ignore_index=IGNORE_INDEX, reduction="none")
#     flat_logits = shifted_logits.view(-1, shifted_logits.size(-1))    # (B*(T-1), V)
#     flat_labels = shifted_labels.view(-1)                             # (B*(T-1),)

#     # Compute token-wise loss
#     flat_loss = loss_function(flat_logits, flat_labels)              # (B*(T-1),)
#     loss_per_sample = flat_loss.view(shifted_labels.shape).sum(dim=-1)  # (B,)

#     # Normalize by number of valid tokens
#     num_token_gt = (shifted_labels != IGNORE_INDEX).sum(dim=-1)      # (B,)
#     avg_losses = loss_per_sample / num_token_gt                      # (B,)
#     normalized_probs = torch.exp(-avg_losses)                        # (B,)
...
def tokenwise_vocab_logprobs(model, batch, grad=False, return_labels=False):
    ...
    for i in range(bsz):
        ...
        if len(actual_indices) == 0:
            ...
            continue
            # warnings.warn(f"Sample {i} has no valid labels, skipping.")
            # continue
        start_idx, end_idx = actual_indices[0].item(), actual_indices[-1].item()
        if start_idx == 0:
            warnings.warn(
        ...
        # log_probs_batch.append(log_probs[i, start_idx : end_idx + 1])
        labels_batch.append(labels[actual_indices])
...
def eval_text_similarity(model, tokenizer, batch, generation_args):
    ...
    # safer slicing, leaving 3 tokens overlap
    context_len = input_ids.shape[-1]
    # slice_start = max(context_len + 3, 0)

    generated_tokens = output[:, context_len:]
    ...
    for i in range(len(gen_texts)):
        ...
        raw_text = raw_text.rsplit('endmodule', 1)[0] + "\n" + "endmodule"
        index = raw_text.find("tb_module")
        if index == -1:
            index = raw_text.find('testbench')
        if index != -1:
            raw_text_temp = raw_text[:index]
            raw_text = raw_text_temp.rsplit('endmodule', 1)[0] + "\n" + "endmodule"
